...
// Function to convert file path to valid SdfPath
std::string convertToSdfPath(const std::string& Path, bool absolute)
{

    std::string unixStylePath = pxr::TfStringReplace(Path, "\\", "/");

    // Stream for building the final path
    std::stringstream sdfPath;
    if (absolute)
    {
        sdfPath << "/"; // Start with a leading slash
    }

    std::string token;
    std::stringstream ss(unixStylePath);

    while (std::getline(ss, token, '/'))
    {
        // Make the token a valid USD identifier
        std::string validToken = pxr::TfMakeValidIdentifier(token);
        sdfPath << validToken << "/";
    }

    // Remove the trailing slash added by the last token
    std::string finalPath = sdfPath.str();
    if (finalPath.back() == '/')
    {
        finalPath.pop_back();
    }
...
std::string makeValidUSDIdentifier(const std::string& name)
{
    ...
    if (validName[0] == '_')
    {
        validName = pxr::TfMakeValidIdentifier("a_" + name);
    }
